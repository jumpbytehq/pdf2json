...
let PDF2JSONUtil = (function () {
	...
	let _parseOnePDFStream = function(callback) {
		...
		outputStream.on('finish', () => callback(null, evtData));
	...
	let _parseOnePDF = function(callback) {
		...
		this.pdfParser.on("pdfParser_dataReady", evtData => {
            // callback = JSON.stringify(evtData);
            // console.log('call back', JSON.stringify(evtData));
            // console.log('evtData: \n', callback);
...
let PDFProcessor = (function () {
    ...
    cls.prototype.start = function(inputFile, callback){
        // if (!this.initialize()) {
        //     console.timeEnd(_PRO_TIMER);
        //     return;
        // }
        // argv.f = "/home/jumpbyte/Downloads/form.pdf";
        argv.f = inputFile;
        console.log('inputFile: ', argv.f);
        try {
            console.log("\n" + _PRO_TIMER);
            ...
            if (inputStatus.isFile()) {
                this.processOneFile(callback);
            }
            else if (inputStatus.isDirectory()) {
                this.processOneDirectory(callback);
            }
        }
        catch(e) {
    ...
    cls.prototype.processOneFile = function (callback) {
        let inputDir = path.dirname(argv.f);
        let inputFile = path.basename(argv.f);

        this.inputCount = 1;
        this.p2j = new PDF2JSONUtil(inputDir, inputFile, this);
        this.p2j.processFile( callback );
    };

    cls.prototype.processFiles = function(inputDir, files, callback) {
        let fId = 0;
        this.p2j = new PDF2JSONUtil(inputDir, files[fId], this);
        this.p2j.processFile( function processPDFFile(err, data) {
            if (err) {
                callback(err);
            }
            else {
    ...
    cls.prototype.processOneDirectory = function (callback) {
        ...
        fs.readdir(inputDir, (err, files) => {
            ...
            if (this.inputCount > 0) {
                this.processFiles(inputDir, pdfFiles, callback);
            }
            else {
